...
class sampleRateChange(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "interpolation", catch_exceptions=True)
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "sampleRateChange")
        ...
        self.interpolation_rate = interpolation_rate = 4
        self.samp_rate_interpolated = samp_rate_interpolated = samp_rate * interpolation_rate
        self.lowPassTaps = lowPassTaps = firdes.low_pass(1.0, samp_rate_interpolated, samp_rate_interpolated / (interpolation_rate * 2), samp_rate_interpolated / (interpolation_rate * 4), window.WIN_HAMMING, 6.76)
        ...
        self.interp_fir_filter_xxx_0 = filter.interp_fir_filter_ccc(interpolation_rate, lowPassTaps)
        ...
        # Connections
        ##################################################
        self.connect((self.analog_sig_source_x_0, 0), (self.interp_fir_filter_xxx_0, 0))
    ...
    def set_interpolation_rate(self, interpolation_rate):
        ...
        self.set_lowPassTaps(firdes.low_pass(1.0, self.samp_rate_interpolated, self.samp_rate_interpolated / (self.interpolation_rate * 2), self.samp_rate_interpolated / (self.interpolation_rate * 4), window.WIN_HAMMING, 6.76))
        self.set_samp_rate_interpolated(self.samp_rate * self.interpolation_rate)
...
def main(top_block_cls=sampleRateChange, options=None):
