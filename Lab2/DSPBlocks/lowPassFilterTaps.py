...
class lowPassFilterTaps(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "filtering", catch_exceptions=True)
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "lowPassFilterTaps")
        ...
        self.lowPassTaps = lowPassTaps = firdes.low_pass(1.0, samp_rate, samp_rate/4, samp_rate/8, window.WIN_HAMMING, 6.76)
        self.n = n = np.arange(0,len(lowPassTaps))
        self.frequencyShift = frequencyShift = np.exp(2j* np.pi * 0.25 * n)
        self.frequency = frequency = 0
        self.boxcarFilter = boxcarFilter = np.ones(8)/8
        self.bandPassTaps = bandPassTaps = lowPassTaps * frequencyShift

        ##################################################
        # Blocks
        ##################################################

        self._frequency_range = qtgui.Range(-samp_rate/2, samp_rate/2, 1, 0, 200)
        ...
        # Connections
        ##################################################
        self.connect((self.analog_sig_source_x_0, 0), (self.freq_xlating_fir_filter_xxx_0, 0))
    ...
    def set_samp_rate(self, samp_rate):
        ...
        self.set_lowPassTaps(firdes.low_pass(1.0, self.samp_rate, self.samp_rate/4, self.samp_rate/8, window.WIN_HAMMING, 6.76))
    ...
    def set_lowPassTaps(self, lowPassTaps):
        ...
        self.set_bandPassTaps(self.lowPassTaps * self.frequencyShift)
        self.set_n(np.arange(0,len(self.lowPassTaps)))
    ...
    def set_n(self, n):
        self.n = n
        self.set_frequencyShift(np.exp(2j* np.pi * 0.25 * self.n))
...
def main(top_block_cls=lowPassFilterTaps, options=None):
